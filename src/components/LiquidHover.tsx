...
void main () {
  ...
  gl_FragColor = vec4(img * opacity, opacity);
    ...
    function setupEvents() {
      ...
      const onResize = () => { resizeCanvas(); initFBOs(); if (imageTexture) gl.bindTexture(gl.TEXTURE_2D, imageTexture); };
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", width: "100%", height: "100%", overflow: "visible", ...style }}>
      <canvas ref={canvasRef} style={{ position: "absolute", top: "-10%", left: "-10%", width: "120%", height: "120%", overflow: "hidden" }} />
    </div>
  );
}
